...
func GetInput(clip ...bool) string {
	var data string
	if len(os.Args) > 1 {
		data = strings.Join(os.Args[1:], " ")
	} else if d, err := GetStdin(); err == nil {
		data = string(d)
	} else if len(clip) > 0 && clip[0] {
		// Attempt to read from clipboard if specified
		// and if reading from stdin failed.
		// This is useful for cases where the user might want to paste input
		// directly into the program.
		// Note: This requires the "github.com/atotto/clipboard" package.
		// Ensure to handle the case where clipboard access might fail.
		// This can happen if the clipboard is empty or if there are permission issues.
